...
function getNewEnglishWord() {
    fetch("/PracticeUnknows/GetNewEnglishWord")
        .then(response => {
            if (!response.ok) {
        ...
        .then(newEnglishWord => {
            if (newEnglishWord && newEnglishWord.trim() !== '') {
                ...
                showNotification("Kelime bulunamadı. Lütfen bilinmeyenler listenize kelime ekleyin.", "error");
            }
        })
        .catch(error => {
...
checkButton.addEventListener("click", function () {
    ...
    fetch("/PracticeUnknows/CheckTranslation?turkishWord=" + encodedTurkishWord + "&englishWord=" + encodedEnglishWord)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
