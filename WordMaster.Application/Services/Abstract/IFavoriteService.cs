...
{
    Task<List<FavoriteWithWordResponse>> GetUserFavoritesAsync(Guid userId);
    Task<ServiceResult<bool>> ToggleFavoriteAsync(ToggleFavoriteRequest request, Guid userId);
    Task<ServiceResult<FavoriteWithWordResponse>> GetFavoriteWithWordAsync(int favoriteId, Guid userId);
